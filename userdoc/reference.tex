...
Each node and way is fetched as a separate record.  For each node or way,
the variable \texttt{TYPE} is set to either \texttt{node}
or \texttt{way} to indicate the type of data, the
variable \texttt{GEOMETRY} is set to the geometry of the
node or way, and the variable \texttt{TAGS} is created

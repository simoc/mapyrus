...
\footnote{Found at \texttt{http://java.sun.com/j2se/1.4/docs/guide/util/logging/overview.html}}
and Java java.util.LogManager API documentation
\footnote{Found at \texttt{http://java.sun.com/j2se/1.4.2/docs/api/java/util/logging/LogManager.html}}
...
\footnote{Found at \texttt{http://java.sun.com/j2se/1.4.2/docs/api/java/sql/DriverManager.html}}
for details.
...
\footnote{Found at \texttt{http://java.sun.com/j2se/1.4.2/docs/api/java/util/regex/Pattern.html}}
...
An argument or expression is assigned to a named variable using the the
...
\texttt{if} \textit{condition} \texttt{then}
\textit{then-commands} ...
\texttt{else}
\textit{else-commands} ...
\texttt{endif}
...
\texttt{if} \textit{condition} \texttt{then}
  \textit{commands} ...
\texttt{elif} \textit{condition} \texttt{then}
  \textit{commands} ...
\texttt{endif}
...
\texttt{while} \textit{condition} \texttt{do}
  \textit{commands} ...
\texttt{done}
...
\texttt{for} \textit{var} \texttt{in} \textit{array} \texttt{do}
  \textit{commands} ...
\texttt{done}
...
\texttt{begin} \textit{name} [\textit{arg1}, ...]
  \textit{commands} ...
\texttt{end}
...
Commands are listed alphabetically.  Some commands accept arguments in several
ways.  For these commands, each combination of arguments is listed.

\subsubsection{addpath}
\texttt{addpath} \textit{geometry-field} [, \textit{geometry-field} ...]
...
\texttt{arc} \textit{direction}, \textit{xCenter}, \textit{yCenter},
\textit{xEnd}, \textit{yEnd}
...
\texttt{color} \textit{name} [, \textit{alpha}]

\texttt{color} \texttt{\"\#}\textit{hexdigits}\texttt{\"} [, \textit{alpha}]

\texttt{color \"rgb\"}, \textit{red}, \textit{green},
\textit{blue} [, \textit{alpha}]

\texttt{color "hsb"}, \textit{hue}, \textit{saturation},
\textit{brightness} [, \textit{alpha}]
...
Table \ref{startupvariables}).  \textit{hexdigits} is a 6 digit hexadecimal
value defining RGB values, as used in HTML pages.  Values for RGB colors and
...
\texttt{dataset} \textit{type}, \textit{name}, \textit{extras}
...
\texttt{jdbc.drivers} (see \ref{startupvariables}).
The JAR file containing the class must be included in the \texttt{-classpath}
option when starting Mapyrus.
...
\texttt{font} \textit{name}, \textit{size}
...
\texttt{import} [\textit{xMin}, \textit{yMin}, \textit{xMax}, \textit{yMax}]

Begin importing records from the dataset set with the \texttt{dataset} command.
After beginning an import, each record is fetched in turn with a \texttt{fetch}
command.  If a bounding rectangle is given and the current dataset contains
geographic fields then only records where the geographic field is inside or
crossing the bounding rectangle are imported.  If no bounding rectangle is
given then all records in the dataset are imported.
...
\texttt{key} \textit{type}, \textit{description}, [\textit{arg1}, \textit{arg2} ...]

Defines an entry for a legend.
The procedure containing this command will be called with arguments
\textit{arg1}, \textit{arg2} ... to display a sample of the symbol
...
\texttt{label} \textit{string} [, \textit{string} ...]
...
(\texttt{\textbackslash}) then labels are displayed as multiple lines, one below the other,
with each line justified independently.

\subsubsection{legend}

\texttt{legend} \textit{size}
...
\texttt{let} \textit{var} = \textit{expression}, \dots

Assigns result of evaluating \textit{expression}
to a variable with name \textit{var}.
The variable is globally accessible, unless defined as local to the
current procedure with a
\texttt{local} command.

Variable \textit{var} is either a simple variable name, an
array, or an array element of the form \textit{var}[\textit{index}].
...
\texttt{linestyle} \textit{width}

\texttt{linestyle} \textit{width}, \textit{cap}, \textit{join}

\texttt{linestyle} \textit{width}, \textit{cap}, \textit{join},
\textit{phase}, \textit{dash length}, ...
...
\texttt{local} \textit{name}, [\textit{name} ...]
...
\texttt{newpage} \textit{format}, \textit{filename}, \textit{width},
\textit{height}, \textit{resolution}, \textit{extras}
...
in millimetres.
\textit{resolution} is given as a dots-per-inch value.
...
\texttt{"| sh \textit{myscript.sh}"}
or
\texttt{"| cmd \textit{myscript.bat}"}
...
\texttt{print} \textit{string} [, \textit{string} ...]
...
\texttt{samplepath} \textit{spacing}, \textit{offset}
...
\texttt{shiftpath} \textit{x}, \textit{y}
...
\texttt{stripepath} \textit{spacing}, \textit{angle}
...
\texttt{worlds} \textit{x1}, \textit{y1}, \textit{x2}, \textit{y2} [, \textit{units}]
...
matching a well-known MIME type (\texttt{html}, \texttt{txt}, \texttt{ps},
\texttt{pdf}, \texttt{zip} or a web image format),
...
startup variable (see \ref{startupvariables} at startup defining

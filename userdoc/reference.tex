...
if running on Linux or a UNIX operating system.
\item
...
\texttt{mif} &
Reads from a MapInfo Interchange Format (MIF)
file with filename \textit{name}.
...
Color and symbol information for each point read from the MIF file
is assigned to variables
named \texttt{SYMBOLSHAPE}, \texttt{SYMBOLCOLOR}, \texttt{SYMBOLSIZE}
and \texttt{SYMBOLFONT}.  For each line and polygon, the variables
named \texttt{PENWIDTH}, \texttt{PENPATTERN}, \texttt{PENCOLOR}
are assigned.  For each polygon, variables
named \texttt{BRUSHPATTERN}, \texttt{BRUSHFORECOLOR}, \texttt{BRUSHBACKCOLOR}
are also assigned with the fill style for the polygon.

\vspace{10pt}
Extras:

\texttt{midfields=\textit{field1},\textit{field2},...}

Comma-separated list of
attribute fields to read from the MID file accompanying the
MIF file.  By default, all fields are read.  Reading fewer attribute
...
\texttt{\$0}.
If filename has suffix \texttt{.gz} or \texttt{.zip}
then it is automatically decompressed as it is read.
...
\subsubsection{roundpath}

\begin{alltt}
roundpath \textit{radius}
\end{alltt}

Rounds corners of current path.
\textit{radius} is the radius for the rounded corners, given in millimetres.
Circular arcs and straight segments that are too short to round are not
changed.
...
millimeters.  \textit{angle} is angle of each line, measured in

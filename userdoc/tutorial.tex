...
The folllowing examples demonstrate how to create output
using Mapyrus.  All examples are included
as files in the \texttt{doc} subdirectory.

If the Ghostscript program or a printer is not available,
then change the \texttt{eps} arguments in 
\texttt{newpage} commands to \texttt{png} to view the output
as image files in a web browser.
...
30mm square Encapsulated PostScript format image named
\texttt{tutorialfirst1.eps}
with the \texttt{newpage} command.
...
The sixth line is a \texttt{stroke} command to draw the
...
the output file \texttt{tutorialfirst1.eps} is completed and Mapyrus exits.
...
the \texttt{legend} command displays the legend.
...
output from Mapyrus through \texttt{ps2pdf} and into a PDF file.  For example,
...
\vspace{10pt}
\framebox{
\texttt{java -classpath \textit{install-dir}/mapyrus.jar org.mapyrus.Mapyrus -s 8410}
}
\vspace{10pt}

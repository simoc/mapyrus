...
\begin{alltt}
java -classpath \textit{install-dir}/mapyrus.jar:\textit{dir}/javaproj-1.0.8.jar \textbackslash
...
\subsection{Running Mapyrus As A Java Servlet Within Apache Tomcat}

The file
\texttt{mapyrus.war}
provided with Mapyrus is a web application containing
the Mapyrus software \texttt{mapyrus.jar} and web application
configuration file \texttt{web.xml}.

To use Mapyrus with Apache Tomcat, the file \texttt{mapyrus.war}
must be deployed to the Apache Tomcat server.  This is most easily
done using one of the following methods:
\begin{itemize}
\item
Copying the file \texttt{mapyrus.war} to the \texttt{webapps}
subdirectory of a Tomcat installation, then starting Tomcat.
\item
Using the Tomcat Manager web interface of a running Tomcat installation
to add the file \texttt{mapyrus.war}.
\end{itemize}

After deploying \texttt{mapyrus.war}, HTTP requests are sent to Mapyrus
using the URL
\begin{verbatim}
http://localhost:8080/mapyrus/
\end{verbatim}

The servlet initialisation parameter \texttt{c1} in the
web application configuration file \texttt{web.xml}
defines the Mapyrus commands that are run when Mapyrus receives an
HTTP request.
Any parameters in URL are set as variables in Mapyrus
before running the Mapyrus commands.
The standard output of the Mapyrus commands is returned from the
HTTP request.

To change the Mapyrus commands, use the following steps:
\begin{enumerate}
\item
Unpack the file \texttt{web.xml} from \texttt{mapyrus.war} using
the command
\begin{verbatim}
jar xf mapyrus.war WEB-INF/web.xml
\end{verbatim}
\item
Edit file \texttt{WEB-INF/web.xml} with a text editor and change
the Mapyrus commands.
\item
Update the file \texttt{mapyrus.war} using command
\begin{verbatim}
jar uf mapyrus.war WEB-INF/web.xml
\end{verbatim}
\item
Remove the old Mapyrus web application from Apache Tomcat, then
add the new Mapyrus web application.
\end{enumerate}

These steps are avoided if the Mapyrus commands in file
\texttt{web.xml} are changed to a single line such as
\begin{verbatim}
include /path/myfile.mapyrus
\end{verbatim}
Then the Mapyrus commands can be edited in a file outside of
Apache Tomcat and tested immediately
without updating \texttt{mapyrus.war} and without redeploying it.
...
to Tomcat by copying it to the Tomcat
\texttt{common/lib} subdirectory before starting Tomcat.

To define more than one set of Mapyrus commands in file \texttt{web.xml},
define each set of commands in a separate \texttt{<init-param>} and give
the parameter \texttt{ipn=\textit{param-name}} in the URL to
Mapyrus.  For example, the following URL runs Mapyrus commands defined
in servlet initialisation parameter \texttt{c2}.
\begin{verbatim}
...
The fourth example creates a PNG image containing a banner,

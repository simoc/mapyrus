...
\verbatiminput{tutorialfirst1-no-newpage.mapyrus}
...
\verbatiminput{tutorialfirst2-no-newpage.mapyrus}
...
\verbatiminput{tutorialfirst3-no-newpage.mapyrus}
...
\verbatiminput{tutorialfirst4-pdf.mapyrus}
...
\texttt{sinewave}, \texttt{chessboard}
and \texttt{logspiral}
commands.
...
\verbatiminput{tutoriallines10.mapyrus}

\begin{figure}[htb]
\includegraphics{tutoriallines10.eps}
...
\verbatiminput{tutorialflowlabel2.mapyrus}

\begin{figure}[htb]
\includegraphics{tutorialflowlabel2.eps}
...
\verbatiminput{tutorialdatasets8.mapyrus}
...
\verbatiminput{tutorialgeoimage4.mapyrus}
...
\verbatiminput{tutorialdatasets9.mapyrus}
...
\verbatiminput{tutorialdatasets10.mapyrus}
...
\subsection{Color For Printing}

RGB colors define exactly the color for display on the screen.
A printer can only approximate these colors.
To define exact colors for printing, use CMYK colors in the
\texttt{COLOR} command when creating PostScript, Encapsulated
PostScript or PDF output.

Color images that are read and displayed using
the \texttt{icon} and \texttt{geoimage} commands are
included in PostScript, Encapsulated
PostScript and PDF output as RGB images.

The following example demonstrates using CMYK colors, with
output shown in Figure \ref{tutorialcolor1}.

\verbatiminput{tutorialcolor1.mapyrus}

\begin{figure}[htb]
\includegraphics{tutorialcolor1.eps}
\caption{CMYK Color}
\label{tutorialcolor1}
\end{figure}
...
\subsection{Formatting In Labels}

It is also possible to manually format text as a sequence
of \texttt{label} commands, using the \texttt{stringwidth}
function to determine when to start a new line.

This is demonstrated in the following example,
with markup tokens in the text being
parsed to change the font or colour between words.
Tab and newline characters also used to control the
positioning of text on the page.
The output of this example
is shown in Figure \ref{tutorialformatting1}.

\verbatiminput{tutorialformatting1.mapyrus}

\begin{figure}[htb]
\includegraphics{tutorialformatting1.eps}
\caption{Label Formatting}
\label{tutorialformatting1}
\end{figure}
...
\verbatiminput{tutorialprotect2.mapyrus}

\begin{figure}[htb]
\includegraphics{tutorialprotect2.eps}
...
\subsection{Creating Groups in PDF Output Files}

When creating PDF output it is possible to define any countinuous
set of output as a named group.

The output included in each group is displayed on the page as normal.
However, PDF viewers such as Adobe Acrobat Reader have menu options to turn
individual groups on and off, producing a layering effect.

The \texttt{pdfgroup} command is used
to define the name, beginning and end of each group, as demonstrated
in the following example

\verbatiminput{tutorialpdfgroup.mapyrus}

\subsection{Mapyrus and JTS Topology Suite Functions}
\label{tutorialjts}

Advanced geometric functions in Mapyrus
(see Figure \ref{functions} on page \pageref{functions})
are provided by the
\textit{JTS Topology Suite},
available from
(\texttt{http://www.tsusiatsoftware.net/jts/main.html}).

If any of these functions are used then the
\textit{JTS Topology Suite} JAR file must be included in the
classpath when running Mapyrus.  The following command demonstrates this:

\begin{alltt}
java -classpath \textit{install-dir}/mapyrus.jar:\textit{jts-dir}/jts-1.13.jar \textbackslash
...
\verbatiminput{tutorialproj1.mapyrus}

\begin{figure}[htb]
\includegraphics{tutorialproj1.eps}
...
\subsection{Using OpenType Fonts In PDF Output}

Each PostScript Type 1 font contains different characters, depending
on the character encoding. However, each character is one byte, limiting
the total number of characters to 256. OpenType fonts avoid this
restriction and enable two byte Unicode characters to be used directly
from Mapyrus.

An OpenType file is a binary file with suffix
\texttt{.otf} and is included in a PDF file created by Mapyrus using the
\textit{extra} \texttt{otffiles} option to the \texttt{newpage} command.

To view the font name and available Unicode characters in an OpenType
font file and to convert other font formats to OpenType, use the
fontforge\footnote{Available from https://fontforge.github.io}
program.
...
\verbatiminput{tutorialhtmlpage3.mapyrus}
...
Enter the following URL to open a web page containing a
...
\subsection{Using Mapyrus JSR 223 Script Engine Interface}

Mapyrus implements JSR 223, the \texttt{javax.script} interfaces enabling
scripting languages to be called from Java.

The interface to Mapyrus is automatically registered when the
\texttt{mapyrus.jar} file is included in the Java classpath.

Create a \texttt{javax.script.ScriptEngine} to run Mapyrus
commands using the following Java class:
...
public class JSR223Demo
{
  public static void main(String []args)
  {
    try
    {
      ...
      ScriptEngine engine = manager.getEngineByName("Mapyrus");
      engine.eval("newpage 'png', 'j.png', 100, 100\n" +
        "color 'yellow'\n" +
        "hexagon 50, 50, 40\n" +
        "fill\n" +
        "endpage");
...
\verbatiminput{tutorialsvg1.mapyrus}
...
Ant build tool.
\item
Java 2 SDK 6, or higher.
\item
JTS Topology Suite 1.13, or higher from \texttt{http://www.tsusiatsoftware.net/jts/main.html}.
\item
...
\item
Servlet API JAR file \texttt{servlet-api.jar} from Apache Tomcat 7
or later.
\item
...
First, checkout the source code from the Mapyrus git server
using the following command.

\begin{verbatim}
git clone https://github.com/simoc/mapyrus
\end{verbatim}
